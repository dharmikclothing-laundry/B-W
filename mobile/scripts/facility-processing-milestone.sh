...
LOG_DIR="$(mktemp -d "${TMPDIR:-/tmp}/bw-8d.XXXXXX")"
REPORT="$REPORT_DIR/8D-PROCESSING-WORKFLOW-$(date +%Y%m%d-%H%M%S).md"
PASS=0
FAIL=0
API_PID=''
...
trap cleanup EXIT
printf '# 8D Facility Processing Workflow\n\nRun: %s\n\n' "$(date '+%Y-%m-%d %H:%M:%S %Z')" >"$REPORT"
note 'Development: local Supabase, fictional accounts and orders, local OTP and mock providers only.'
note '8D reuses existing atomic processing procedures and approved lifecycle transitions. Facility operators run Wash, Dry, Iron, Fold and Pack-prep in sequence.'
note '8C measured-intake and discrepancy guards remain mandatory. Processing records retain stage start/completion timestamps and both operator identities.'
note 'Quality control and rewash were not started. Fictional processing records are cleaned after local acceptance.'
...
gate 'clean 8C baseline and reviewed 8D files only' "$MOBILE" python3 scripts/qa-8d-worktree.py verify
if (( FAIL > 0 )); then finish; exit 1; fi
note '## Changed files'
python3 "$MOBILE/scripts/qa-8d-worktree.py" verify | rg '^(backend|mobile): ' | sed 's/^/- /' >>"$REPORT"
...
gate 'automation and acceptance syntax' "$ROOT" bash -c 'bash -n bw mobile/scripts/facility-processing-milestone.sh && python3 -c '\''import ast,pathlib; ast.parse(pathlib.Path("mobile/scripts/qa-8d-worktree.py").read_text())'\'' && node --check backend/scripts/7f-functional-acceptance.cjs'
gate 'backend Facility processing and authorization targeted tests' "$BACKEND" npm test -- --runInBand --watchman=false facility.service.spec.ts facility.verification.spec.ts
gate 'mobile Facility processing and API tests' "$MOBILE" npm test -- --runInBand --watch=false --watchman=false --silent FacilityVerificationScreen FacilityProcessingPanel facilityProcessingApi
...
note '## Authenticated local processing acceptance'
if lsof -tiTCP:3001 -sTCP:LISTEN >/dev/null 2>&1; then
  ...
  API_PID=$!
  gate 'local 8D API health' "$ROOT" bash -c 'for i in {1..30}; do curl -fsS --max-time 2 http://127.0.0.1:3001/v1/health >/dev/null 2>&1 && exit 0; sleep 2; done; exit 1'
  if curl -fsS --max-time 2 http://127.0.0.1:3001/v1/health >/dev/null 2>&1; then
    gate 'fictional verified-intake processing sequence and audit acceptance' "$BACKEND" env BW_8D_ACCEPTANCE=1 node scripts/7f-functional-acceptance.cjs
  fi
fi
if (( FAIL == 0 )); then
  note '## Final checkpoint'
  gate 'final changed-file and diff review' "$MOBILE" python3 scripts/qa-8d-worktree.py verify
  if (( FAIL == 0 )); then
    if gate 'checkpoint and clean Git trees' "$MOBILE" python3 scripts/qa-8d-worktree.py commit; then CHECKPOINT='CREATED'; fi
  fi
fi
finish
(( FAIL == 0 ))
