...
LOG_DIR="$(mktemp -d "${TMPDIR:-/tmp}/bw-9g.XXXXXX")"
REPORT="$REPORT_DIR/9G-ADMIN-GROWTH-PROGRAMS-$(date +%Y%m%d-%H%M%S).md"
...
finish() {
  ...
  note 'Fictional local offers and package contract retained inactive. Original local program rules restored after acceptance.'
  note "Report: $REPORT"
}
trap 'if [[ -n "$API_PID" ]]; then kill "$API_PID" 2>/dev/null || true; wait "$API_PID" 2>/dev/null || true; fi' EXIT
printf '# 9G Admin Coupons, Offers, Loyalty, Referrals and Packages\n\nRun: %s\n\n' "$(date '+%Y-%m-%d %H:%M:%S %Z')" >"$REPORT"
note 'Scope: Admin offer and package management, atomic limited-use coupon redemption, sold-contract locks, referral and loyalty rules, customer checkout consistency and immutable Admin audit.'
...
gate 'clean 9F baselines and reviewed 9G files' "$MOBILE" python3 scripts/qa-9g-worktree.py verify
if (( FAIL > 0 )); then finish; exit 1; fi
note '## Changed files'
python3 "$MOBILE/scripts/qa-9g-worktree.py" verify | rg '^(backend|mobile): ' | sed 's/^/- /' >>"$REPORT"
...
gate 'runner and fixture syntax' "$ROOT" bash -c 'bash -n bw mobile/scripts/admin-growth-milestone.sh && node --check backend/scripts/9g-functional-acceptance.cjs && python3 -c '\''import ast,pathlib; ast.parse(pathlib.Path("mobile/scripts/qa-9g-worktree.py").read_text())'\'''
gate 'backend Admin growth, coupon, checkout and package tests' "$BACKEND" npm test -- --runInBand --watchman=false admin-growth.service.spec.ts growth.service.spec.ts orders.service.spec.ts packages.service.spec.ts
gate 'backend HTTP Admin RBAC integration' "$BACKEND" npm run test:e2e -- --runInBand --watchman=false
gate 'mobile Admin growth and customer checkout tests' "$MOBILE" npm test -- --runInBand --watch=false --watchman=false --silent AdminGrowthScreen LoyaltySection OrderReviewScreen CouponSection
...
if lsof -tiTCP:3001 -sTCP:LISTEN >/dev/null 2>&1; then
  ...
  gate 'local 9G API health' "$ROOT" bash -c 'for i in {1..30}; do curl -fsS --max-time 2 http://127.0.0.1:3001/v1/health >/dev/null 2>&1 && exit 0; sleep 2; done; exit 1'
  if curl -fsS --max-time 2 http://127.0.0.1:3001/v1/health >/dev/null 2>&1; then
    gate 'fictional Admin offer, limited coupon, sold package and program rule acceptance' "$BACKEND" node scripts/9g-functional-acceptance.cjs
  fi
fi
note '## Final checkpoint'
gate 'final changed-file and diff review' "$MOBILE" python3 scripts/qa-9g-worktree.py verify
if (( FAIL == 0 )); then
  if gate 'checkpoint and clean Git trees' "$MOBILE" python3 scripts/qa-9g-worktree.py commit; then CHECKPOINT='CREATED'; fi
fi
finish
(( FAIL == 0 ))
