...
LOG_DIR="$(mktemp -d "${TMPDIR:-/tmp}/bw-9h.XXXXXX")"
REPORT="$REPORT_DIR/9H-ADMIN-FACILITY-OVERSIGHT-$(date +%Y%m%d-%H%M%S).md"
...
finish() {
  ...
  note 'No Facility lifecycle mutations were made by 9H acceptance. Monitoring thresholds are operational indicators, not contractual SLAs.'
  note "Report: $REPORT"
}
trap 'if [[ -n "$API_PID" ]]; then kill "$API_PID" 2>/dev/null || true; wait "$API_PID" 2>/dev/null || true; fi' EXIT
printf '# 9H Admin Facility Oversight and Operational Monitoring\n\nRun: %s\n\n' "$(date '+%Y-%m-%d %H:%M:%S %Z')" >"$REPORT"
note 'Scope: read-only Admin cross-Facility workload, intake, processing, QC, packing, QR audit, age indicators and order history.'
note 'All 9H endpoints are read-only. Development uses local Supabase and mock providers only; no Production services.'
note '## Preflight and safety'
gate 'local/mock provider separation' "$MOBILE" python3 scripts/qa-release-audit.py local-safety
gate 'clean 9G baselines and reviewed 9H files' "$MOBILE" python3 scripts/qa-9h-worktree.py verify
if (( FAIL > 0 )); then finish; exit 1; fi
note '## Changed files'
python3 "$MOBILE/scripts/qa-9h-worktree.py" verify | rg '^(backend|mobile): ' | sed 's/^/- /' >>"$REPORT"
...
gate 'runner and fixture syntax' "$ROOT" bash -c 'bash -n bw mobile/scripts/admin-facility-oversight-milestone.sh && node --check backend/scripts/9h-functional-acceptance.cjs && python3 -c '\''import ast,pathlib; ast.parse(pathlib.Path("mobile/scripts/qa-9h-worktree.py").read_text())'\'''
gate 'backend Admin Facility oversight tests' "$BACKEND" npm test -- --runInBand --watchman=false admin-facility-oversight.service.spec.ts
gate 'backend HTTP Admin RBAC integration' "$BACKEND" npm run test:e2e -- --runInBand --watchman=false
gate 'mobile Admin Facility oversight tests' "$MOBILE" npm test -- --runInBand --watch=false --watchman=false --silent AdminFacilityOversightScreen AdminDashboardScreen
...
if lsof -tiTCP:3001 -sTCP:LISTEN >/dev/null 2>&1; then
  ...
  gate 'local 9H API health' "$ROOT" bash -c 'for i in {1..30}; do curl -fsS --max-time 2 http://127.0.0.1:3001/v1/health >/dev/null 2>&1 && exit 0; sleep 2; done; exit 1'
  if curl -fsS --max-time 2 http://127.0.0.1:3001/v1/health >/dev/null 2>&1; then
    gate 'fictional Admin cross-Facility read-only acceptance' "$BACKEND" node scripts/9h-functional-acceptance.cjs
  fi
fi
note '## Final checkpoint'
gate 'final changed-file and diff review' "$MOBILE" python3 scripts/qa-9h-worktree.py verify
if (( FAIL == 0 )); then
  if gate 'checkpoint and clean Git trees' "$MOBILE" python3 scripts/qa-9h-worktree.py commit; then CHECKPOINT='CREATED'; fi
fi
finish
(( FAIL == 0 ))
