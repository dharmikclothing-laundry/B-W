...
LOG_DIR="$(mktemp -d "${TMPDIR:-/tmp}/bw-8e.XXXXXX")"
REPORT="$REPORT_DIR/8E-REWASH-QC-$(date +%Y%m%d-%H%M%S).md"
PASS=0
FAIL=0
API_PID=''
...
trap cleanup EXIT
printf '# 8E Rewash and Quality Control\n\nRun: %s\n\n' "$(date '+%Y-%m-%d %H:%M:%S %Z')" >"$REPORT"
note 'Development: local Supabase, fictional accounts and orders, local OTP and mock providers only.'
note '8E reuses existing atomic QC and processing transitions; only an active Facility Manager may decide QC.'
note 'Failed QC requires a defect and reason. Immutable QC decision records and cycle numbers retain original processing history; no more than two failed/rewash cycles are permitted.'
note 'Ready-for-Delivery requires a passing Manager QC decision. Fictional acceptance records are removed after verification.'
...
gate 'clean 8D baseline and reviewed 8E files only' "$MOBILE" python3 scripts/qa-8e-worktree.py verify
if (( FAIL > 0 )); then finish; exit 1; fi
note '## Changed files'
python3 "$MOBILE/scripts/qa-8e-worktree.py" verify | rg '^(backend|mobile): ' | sed 's/^/- /' >>"$REPORT"
...
gate 'automation and acceptance syntax' "$ROOT" bash -c 'bash -n bw mobile/scripts/facility-quality-milestone.sh && python3 -c '\''import ast,pathlib; ast.parse(pathlib.Path("mobile/scripts/qa-8e-worktree.py").read_text())'\'' && node --check backend/scripts/7f-functional-acceptance.cjs'
gate 'backend Facility QC and authorization targeted tests' "$BACKEND" npm test -- --runInBand --watchman=false facility.service.spec.ts facility.verification.spec.ts
gate 'mobile Facility QC and API tests' "$MOBILE" npm test -- --runInBand --watch=false --watchman=false --silent FacilityVerificationScreen FacilityProcessingPanel facilityProcessingApi
...
note '## Authenticated local QC and rewash acceptance'
if lsof -tiTCP:3001 -sTCP:LISTEN >/dev/null 2>&1; then
  ...
  API_PID=$!
  gate 'local 8E API health' "$ROOT" bash -c 'for i in {1..30}; do curl -fsS --max-time 2 http://127.0.0.1:3001/v1/health >/dev/null 2>&1 && exit 0; sleep 2; done; exit 1'
  if curl -fsS --max-time 2 http://127.0.0.1:3001/v1/health >/dev/null 2>&1; then
    gate 'fictional Manager QC, rewash cap and final readiness acceptance' "$BACKEND" env BW_8E_ACCEPTANCE=1 node scripts/7f-functional-acceptance.cjs
  fi
fi
if (( FAIL == 0 )); then
  note '## Final checkpoint'
  gate 'final changed-file and diff review' "$MOBILE" python3 scripts/qa-8e-worktree.py verify
  if (( FAIL == 0 )); then
    if gate 'checkpoint and clean Git trees' "$MOBILE" python3 scripts/qa-8e-worktree.py commit; then CHECKPOINT='CREATED'; fi
  fi
fi
finish
(( FAIL == 0 ))
