...
LOG_DIR="$(mktemp -d "${TMPDIR:-/tmp}/bw-8f.XXXXXX")"
REPORT="$REPORT_DIR/8F-PACKING-READY-DELIVERY-$(date +%Y%m%d-%H%M%S).md"
PASS=0
FAIL=0
API_PID=''
...
trap cleanup EXIT
printf '# 8F Final Packing and Ready for Delivery\n\nRun: %s\n\n' "$(date '+%Y-%m-%d %H:%M:%S %Z')" >"$REPORT"
note 'Development: local Supabase, fictional accounts and orders, local OTP and mock providers only.'
note '8F records per-item packed counts against Facility-verified intake, parcel ID, operator, timestamp and notes for the current processing cycle.'
note 'Newly received orders cannot reach Ready-for-Delivery until final packing is confirmed and Manager QC passes through the existing atomic lifecycle.'
note 'Development acceptance checks missing items, duplicate packing, wrong Facility, QC gate, delivery assignment and fictional fixture cleanup.'
...
gate 'clean 8E baseline and reviewed 8F files only' "$MOBILE" python3 scripts/qa-8f-worktree.py verify
if (( FAIL > 0 )); then finish; exit 1; fi
note '## Changed files'
python3 "$MOBILE/scripts/qa-8f-worktree.py" verify | rg '^(backend|mobile): ' | sed 's/^/- /' >>"$REPORT"
...
gate 'automation and acceptance syntax' "$ROOT" bash -c 'bash -n bw mobile/scripts/facility-packing-milestone.sh && python3 -c '\''import ast,pathlib; ast.parse(pathlib.Path("mobile/scripts/qa-8f-worktree.py").read_text())'\'' && node --check backend/scripts/7f-functional-acceptance.cjs'
gate 'backend Facility packing and QC targeted tests' "$BACKEND" npm test -- --runInBand --watchman=false facility.packing.spec.ts facility.service.spec.ts
gate 'mobile Facility packing and QC tests' "$MOBILE" npm test -- --runInBand --watch=false --watchman=false --silent FacilityVerificationScreen FacilityPackingPanel facilityPackingApi FacilityProcessingPanel
...
note '## Authenticated local packing and readiness acceptance'
if lsof -tiTCP:3001 -sTCP:LISTEN >/dev/null 2>&1; then
  ...
  API_PID=$!
  gate 'local 8F API health' "$ROOT" bash -c 'for i in {1..30}; do curl -fsS --max-time 2 http://127.0.0.1:3001/v1/health >/dev/null 2>&1 && exit 0; sleep 2; done; exit 1'
  if curl -fsS --max-time 2 http://127.0.0.1:3001/v1/health >/dev/null 2>&1; then
    gate 'fictional final packing, QC readiness and delivery assignment acceptance' "$BACKEND" env BW_8F_ACCEPTANCE=1 node scripts/7f-functional-acceptance.cjs
  fi
fi
if (( FAIL == 0 )); then
  note '## Final checkpoint'
  gate 'final changed-file and diff review' "$MOBILE" python3 scripts/qa-8f-worktree.py verify
  if (( FAIL == 0 )); then
    if gate 'checkpoint and clean Git trees' "$MOBILE" python3 scripts/qa-8f-worktree.py commit; then CHECKPOINT='CREATED'; fi
  fi
fi
finish
(( FAIL == 0 ))
