...
LOG_DIR="$(mktemp -d "${TMPDIR:-/tmp}/bw-7f.XXXXXX")"
REPORT="$REPORT_DIR/7F-FACILITY-HANDOFF-$(date +%Y%m%d-%H%M%S).md"
PASS=0
FAIL=0
API_PID=''
...
gate() {
  ...
  if (cd "$directory" && "$@") >"$output" 2>&1; then
    note "  - PASS: $label"
    PASS=$((PASS+1))
    if [[ "$label" == 'authenticated local 7F facility handoff acceptance' ]]; then
...
printf '# 7F Facility Handoff\n\nRun: %s\n\n' "$(date '+%Y-%m-%d %H:%M:%S %Z')" >"$REPORT"
note 'Contract: existing start_facility_transit_atomic and receive_facility_order_atomic procedures. Facility employee confirms QR receipt; no direct order-status writes or new backend procedure.'
note 'Development/UAT uses local Supabase and mock Maps/payment providers. No Production service is used.'
note '## Preflight and safety'
gate 'local/mock providers only' "$MOBILE" python3 scripts/qa-release-audit.py local-safety
gate '7E baselines and reviewed 7F changes only' "$MOBILE" python3 scripts/qa-7f-worktree.py verify
if (( FAIL > 0 )); then finish; exit 1; fi
note ''
note '## Changed files'
python3 "$MOBILE/scripts/qa-7f-worktree.py" verify | rg '^(backend|mobile): ' | sed 's/^/- /' >>"$REPORT"
...
gate 'runner and acceptance syntax' "$ROOT" bash -c 'bash -n bw mobile/scripts/driver-facility-handoff-milestone.sh && node --check backend/scripts/7f-functional-acceptance.cjs'
gate 'backend Driver, transit and facility receipt tests' "$BACKEND" npm test -- --runInBand --watchman=false drivers.service.spec.ts logistics.service.spec.ts facility.service.spec.ts
gate 'mobile Driver handoff and API tests' "$MOBILE" npm test -- --runInBand --watch=false --watchman=false --silent DriverJobDetailScreen driverAssignmentsApi
...
if (( FAIL == 0 )); then
  ...
  if lsof -tiTCP:3001 -sTCP:LISTEN >/dev/null 2>&1; then
    ...
    if (( FAIL == 0 )); then
      gate 'authenticated local 7F facility handoff acceptance' "$BACKEND" node scripts/7f-functional-acceptance.cjs
    fi
  fi
fi
...
if (( FAIL == 0 )); then
  ...
  gate 'final changed-file and diff review' "$MOBILE" python3 scripts/qa-7f-worktree.py verify
  if (( FAIL == 0 )); then
    gate 'checkpoint and clean Git trees' "$MOBILE" python3 scripts/qa-7f-worktree.py commit
