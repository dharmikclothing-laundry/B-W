...
LOG_DIR="$(mktemp -d "${TMPDIR:-/tmp}/bw-7e.XXXXXX")"
REPORT="$REPORT_DIR/7E-PICKUP-OTP-GARMENT-HANDOVER-$(date +%Y%m%d-%H%M%S).md"
PASS=0
FAIL=0
API_PID=''
...
gate() {
  ...
  if (cd "$directory" && "$@") >"$output" 2>&1; then
    note "  - PASS: $label"
    PASS=$((PASS+1))
    if [[ "$label" == 'authenticated local 7E pickup handover acceptance' ]]; then
...
printf '# 7E Pickup OTP and Garment Handover\n\nRun: %s\n\n' "$(date '+%Y-%m-%d %H:%M:%S %Z')" >"$REPORT"
note 'Contract: existing customer-issued local pickup OTP, existing atomic pickup completion procedure, and existing order item fields. No new backend procedure, pickup photo field, or direct order-status write.'
note 'Development/UAT OTP delivery is local mock only; Production issuance remains blocked by the existing backend.'
note '## Preflight and safety'
gate 'local/mock providers only' "$MOBILE" python3 scripts/qa-release-audit.py local-safety
gate '7D baselines and reviewed 7E changes only' "$MOBILE" python3 scripts/qa-7e-worktree.py verify
if (( FAIL > 0 )); then finish; exit 1; fi
note ''
note '## Changed files'
python3 "$MOBILE/scripts/qa-7e-worktree.py" verify | rg '^(backend|mobile): ' | sed 's/^/- /' >>"$REPORT"
...
gate 'runner and acceptance syntax' "$ROOT" bash -c 'bash -n bw mobile/scripts/driver-pickup-milestone.sh && node --check backend/scripts/7e-functional-acceptance.cjs'
gate 'backend pickup OTP ownership, expiry, reuse, and item tests' "$BACKEND" npm test -- --runInBand --watchman=false otp.service.spec.ts drivers.service.spec.ts
gate 'mobile pickup handover and OTP integration tests' "$MOBILE" npm test -- --runInBand --watch=false --watchman=false --silent DriverJobDetailScreen driverAssignmentsApi orderOtpApi
...
if (( FAIL == 0 )); then
  ...
  if lsof -tiTCP:3001 -sTCP:LISTEN >/dev/null 2>&1; then
    ...
    API_PID=$!
    gate 'local 7E API health' "$ROOT" bash -c 'for i in {1..30}; do curl -fsS --max-time 2 http://127.0.0.1:3001/v1/health >/dev/null 2>&1 && exit 0; sleep 2; done; exit 1'
    if (( FAIL == 0 )); then
      gate 'authenticated local 7E pickup handover acceptance' "$BACKEND" node scripts/7e-functional-acceptance.cjs
    fi
  fi
fi
...
if (( FAIL == 0 )); then
  ...
  gate 'final changed-file and diff review' "$MOBILE" python3 scripts/qa-7e-worktree.py verify
  if (( FAIL == 0 )); then
    gate 'checkpoint and clean Git trees' "$MOBILE" python3 scripts/qa-7e-worktree.py commit
