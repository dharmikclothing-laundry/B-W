...
LOG_DIR="$(mktemp -d "${TMPDIR:-/tmp}/bw-9i.XXXXXX")"
REPORT="$REPORT_DIR/9I-ADMIN-ANALYTICS-AUDIT-$(date +%Y%m%d-%H%M%S).md"
...
finish() {
  ...
  note '9I acceptance used read-only analytics and audit endpoints. Operational and audit record counts remained unchanged. Mobile external sharing of sensitive CSV was excluded after automatic approval review; authenticated Admin CSV downloads remain available.'
  note "Report: $REPORT"
}
trap 'if [[ -n "$API_PID" ]]; then kill "$API_PID" 2>/dev/null || true; wait "$API_PID" 2>/dev/null || true; fi' EXIT
printf '# 9I Admin Analytics, Reports and Audit Logs\n\nRun: %s\n\n' "$(date '+%Y-%m-%d %H:%M:%S %Z')" >"$REPORT"
note 'Scope: revenue, order trends, cancellations, claims, refunds, Driver and Facility performance, growth usage, unified Admin audit search, and protected CSV reports.'
note 'All 9I report, audit and CSV endpoints are GET only. Development uses local Supabase and mock providers only; no Production services.'
note '## Preflight and safety'
gate 'local/mock provider separation' "$MOBILE" python3 scripts/qa-release-audit.py local-safety
gate 'clean 9H baselines and reviewed 9I files' "$MOBILE" python3 scripts/qa-9i-worktree.py verify
if (( FAIL > 0 )); then finish; exit 1; fi
note '## Changed files'
python3 "$MOBILE/scripts/qa-9i-worktree.py" verify | rg '^(backend|mobile): ' | sed 's/^/- /' >>"$REPORT"
...
gate 'runner and fixture syntax' "$ROOT" bash -c 'bash -n bw mobile/scripts/admin-analytics-milestone.sh && node --check backend/scripts/9i-functional-acceptance.cjs && python3 -c '\''import ast,pathlib; ast.parse(pathlib.Path("mobile/scripts/qa-9i-worktree.py").read_text())'\'''
gate 'backend Admin analytics and audit tests' "$BACKEND" npm test -- --runInBand --watchman=false analytics-reports.service.spec.ts analytics.service.spec.ts
gate 'backend HTTP Admin RBAC integration' "$BACKEND" npm run test:e2e -- --runInBand --watchman=false
gate 'mobile Admin analytics and audit tests' "$MOBILE" npm test -- --runInBand --watch=false --watchman=false --silent AdminReportsScreen adminReportsApi AdminDashboardScreen
...
if lsof -tiTCP:3001 -sTCP:LISTEN >/dev/null 2>&1; then
  ...
  gate 'local 9I API health' "$ROOT" bash -c 'for i in {1..30}; do curl -fsS --max-time 2 http://127.0.0.1:3001/v1/health >/dev/null 2>&1 && exit 0; sleep 2; done; exit 1'
  if curl -fsS --max-time 2 http://127.0.0.1:3001/v1/health >/dev/null 2>&1; then
    gate 'fictional Admin read-only report, audit and CSV acceptance' "$BACKEND" node scripts/9i-functional-acceptance.cjs
  fi
fi
note '## Final checkpoint'
gate 'final changed-file and diff review' "$MOBILE" python3 scripts/qa-9i-worktree.py verify
if (( FAIL == 0 )); then
  if gate 'checkpoint and clean Git trees' "$MOBILE" python3 scripts/qa-9i-worktree.py commit; then CHECKPOINT='CREATED'; fi
fi
finish
(( FAIL == 0 ))
