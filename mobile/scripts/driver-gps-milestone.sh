...
LOG_DIR="$(mktemp -d "${TMPDIR:-/tmp}/bw-7d.XXXXXX")"
REPORT="$REPORT_DIR/7D-PICKUP-NAVIGATION-LIVE-GPS-$(date +%Y%m%d-%H%M%S).md"
PASS=0
FAIL=0
API_PID=''
...
gate() {
  ...
  if (cd "$directory" && "$@") >"$output" 2>&1; then
    note "  - PASS: $label"
    PASS=$((PASS+1))
    if [[ "$label" == 'authenticated local 7D GPS and customer tracking acceptance' ]]; then
...
printf '# 7D Pickup Navigation and Live GPS\n\nRun: %s\n\n' "$(date '+%Y-%m-%d %H:%M:%S %Z')" >"$REPORT"
note 'Background design: Android foreground location service with a persistent trip notification; iOS Core Location background mode. Publishing stops on inactive trip responses and logout.'
note 'Physical acceptance requires recorded Android and iOS GPS evidence while native navigation is foreground, plus stop-on-trip-end evidence.'
note '## Preflight and safety'
gate 'local/mock providers only' "$MOBILE" python3 scripts/qa-release-audit.py local-safety
gate '7C baselines and reviewed 7D changes only' "$MOBILE" python3 scripts/qa-7d-worktree.py verify
if (( FAIL > 0 )); then finish; exit 1; fi
note ''
note '## Changed files'
python3 "$MOBILE/scripts/qa-7d-worktree.py" verify | rg '^(backend|mobile): ' | sed 's/^/- /' >>"$REPORT"
...
gate 'runner and acceptance syntax' "$ROOT" bash -c 'bash -n bw mobile/scripts/driver-gps-milestone.sh && node --check backend/scripts/7d-functional-acceptance.cjs && node --check backend/scripts/7d-device-fixture.cjs'
gate 'backend 7D GPS authorization and tracking tests' "$BACKEND" npm test -- --runInBand --watchman=false logistics.service.spec.ts logistics.assignments.spec.ts drivers.service.spec.ts
gate 'mobile 7D navigation, GPS, and permission tests' "$MOBILE" npm test -- --runInBand --watch=false --watchman=false --silent DriverDashboardScreen DriverJobDetailScreen driverAssignmentsApi driverTripBackground
...
if (( FAIL == 0 )); then
  ...
  if lsof -tiTCP:3001 -sTCP:LISTEN >/dev/null 2>&1; then
    ...
    API_PID=$!
    gate 'local 7D API health' "$ROOT" bash -c 'for i in {1..30}; do curl -fsS --max-time 2 http://127.0.0.1:3001/v1/health >/dev/null 2>&1 && exit 0; sleep 2; done; exit 1'
    if (( FAIL == 0 )); then
      gate 'authenticated local 7D GPS and customer tracking acceptance' "$BACKEND" node scripts/7d-functional-acceptance.cjs
    fi
  fi
fi
...
if (( FAIL == 0 )); then
  ...
  gate 'physical background GPS continuity on Android and iOS' "$MOBILE" python3 scripts/qa-7d-device-acceptance.py
  gate 'final changed-file and diff review' "$MOBILE" python3 scripts/qa-7d-worktree.py verify
  if (( FAIL == 0 )); then
    gate 'checkpoint and clean Git trees' "$MOBILE" python3 scripts/qa-7d-worktree.py commit
