#!/usr/bin/env python3
"""Allowlist and checkpoint 7F facility handoff."""
import pathlib
import subprocess
import sys
...
repos = {
    ...
    root / 'mobile': {
        ...
        'scripts/driver-facility-handoff-milestone.sh',
        'scripts/qa-7f-worktree.py',
    },
}
...
def verify():
    for repo, baseline in zip(repos, ('4b7d7ee', 'e684d93')):
        if git(repo, 'cat-file', '-t', baseline).strip() != 'commit':
            raise RuntimeError(f'{repo.name}: 7E baseline missing')
        if git(repo, 'diff', '--cached', '--name-only'):
            raise RuntimeError(f'{repo.name}: pre-staged files present')
        files = changed(repo)
        if not files:
            raise RuntimeError(f'{repo.name}: expected 7F changes absent')
        git(repo, 'diff', '--check')
        for path in files:
            print(f'{repo.name}: {path}')
    print('PASS: reviewed 7F changes only')
...
def checkpoint():
    ...
    for repo in repos:
        git(repo, 'commit', '-m', 'checkpoint(7F): atomic facility handoff')
...
try:
    if sys.argv[1:] == ['verify']:
        ...
        raise RuntimeError('Usage: qa-7f-worktree.py verify|commit')
