...
LOG_DIR="$(mktemp -d "${TMPDIR:-/tmp}/bw-9b.XXXXXX")"
REPORT="$REPORT_DIR/9B-ADMIN-CUSTOMER-ORDER-MANAGEMENT-$(date +%Y%m%d-%H%M%S).md"
PASS=0
FAIL=0
API_PID=''
...
printf '# 9B Admin Customer and Order Management\n\nRun: %s\n\n' "$(date '+%Y-%m-%d %H:%M:%S %Z')" >"$REPORT"
note 'Scope: Admin-authorized customer search/profile/history, order timeline/QR, pricing/GST, payment/refund/claims visibility and atomic Admin cancellation. Existing customer ownership/RLS and immutable payment history preserved.'
note 'Development: local Supabase, fictional test fixtures and mock providers only. No Production services.'
note 'UAT / Production activation gates: DEFERRED TO PLATFORM UAT.'
note '## Preflight and safety'
gate 'local/mock provider separation' "$MOBILE" python3 scripts/qa-release-audit.py local-safety
gate 'reviewed 9B baselines and QR lookup files' "$MOBILE" python3 scripts/qa-9b-worktree.py verify
if (( FAIL > 0 )); then finish; exit 1; fi
note '## Changed files'
python3 "$MOBILE/scripts/qa-9b-worktree.py" verify | rg '^(backend|mobile): ' | sed 's/^/- /' >>"$REPORT"
...
gate 'runner and fixture syntax' "$ROOT" bash -c 'bash -n bw mobile/scripts/admin-customer-orders-milestone.sh && node --check backend/scripts/9b-functional-acceptance.cjs && python3 -c '\''import ast,pathlib; ast.parse(pathlib.Path("mobile/scripts/qa-9b-worktree.py").read_text())'\'''
gate 'backend Admin management targeted tests' "$BACKEND" npm test -- --runInBand --watchman=false admin-management.service.spec.ts orders.service.spec.ts payments.customer-flows.spec.ts claims.service.spec.ts
gate 'backend HTTP Admin RBAC integration' "$BACKEND" npm run test:e2e -- --runInBand --watchman=false
gate 'mobile Admin management screens and API tests' "$MOBILE" npm test -- --runInBand --watch=false --watchman=false --silent adminManagementApi AdminManagementScreens AdminCustomersScreen AdminCustomerDetailScreen AdminDashboardScreen
...
if lsof -tiTCP:3001 -sTCP:LISTEN >/dev/null 2>&1; then
  ...
  gate 'local 9B API health' "$ROOT" bash -c 'for i in {1..30}; do curl -fsS --max-time 2 http://127.0.0.1:3001/v1/health >/dev/null 2>&1 && exit 0; sleep 2; done; exit 1'
  if curl -fsS --max-time 2 http://127.0.0.1:3001/v1/health >/dev/null 2>&1; then
    gate 'fictional Admin customer, order, QR and atomic cancellation acceptance' "$BACKEND" node scripts/9b-functional-acceptance.cjs
  fi
fi
note '## Final checkpoint'
gate 'final changed-file and diff review' "$MOBILE" python3 scripts/qa-9b-worktree.py verify
if (( FAIL == 0 )); then
  if gate 'checkpoint and clean Git trees' "$MOBILE" python3 scripts/qa-9b-worktree.py commit; then CHECKPOINT='CREATED'; fi
fi
finish
(( FAIL == 0 ))
