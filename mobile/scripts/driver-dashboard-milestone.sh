...
LOG_DIR="$(mktemp -d "${TMPDIR:-/tmp}/bw-7b.XXXXXX")"
REPORT="$REPORT_DIR/7B-DRIVER-DASHBOARD-ASSIGNMENTS-$(date +%Y%m%d-%H%M%S).md"
PASS=0
FAIL=0
API_PID=''
...
gate() {
  ...
  if (cd "$directory" && "$@") >"$output" 2>&1; then
    note "  - PASS: $label"
    PASS=$((PASS+1))
    if [[ "$label" == 'authenticated local Driver dashboard acceptance' ]]; then
...
printf '# 7B Driver Dashboard and Assignments\n\nRun: %s\n\n' "$(date '+%Y-%m-%d %H:%M:%S %Z')" >"$REPORT"
note '## Preflight and safety'
gate 'local/mock providers only' "$MOBILE" python3 scripts/qa-release-audit.py local-safety
gate '7A baselines and reviewed 7B changes only' "$MOBILE" python3 scripts/qa-7b-worktree.py verify
if (( FAIL > 0 )); then finish; exit 1; fi
note ''
note '## Changed files'
python3 "$MOBILE/scripts/qa-7b-worktree.py" verify | rg '^(backend|mobile): ' | sed 's/^/- /' >>"$REPORT"
...
gate 'runner and acceptance syntax' "$ROOT" bash -c 'bash -n bw mobile/scripts/driver-dashboard-milestone.sh && node --check backend/scripts/7b-functional-acceptance.cjs'
gate 'backend assignment authorization tests' "$BACKEND" npm test -- --runInBand --watchman=false drivers.service.spec.ts
gate 'mobile dashboard and detail tests' "$MOBILE" npm test -- --runInBand --watch=false --watchman=false --silent driverDashboard driverJobDetail driverAssignmentsApi
...
if (( FAIL == 0 )); then
  ...
  if lsof -tiTCP:3001 -sTCP:LISTEN >/dev/null 2>&1; then
    ...
    API_PID=$!
    gate 'local 7B API health' "$ROOT" bash -c 'for i in {1..30}; do curl -fsS --max-time 2 http://127.0.0.1:3001/v1/health >/dev/null 2>&1 && exit 0; sleep 2; done; exit 1'
    if (( FAIL == 0 )); then
      gate 'authenticated local Driver dashboard acceptance' "$BACKEND" node scripts/7b-functional-acceptance.cjs
    fi
  fi
fi
...
if (( FAIL == 0 )); then
  ...
  gate 'final changed-file and diff review' "$MOBILE" python3 scripts/qa-7b-worktree.py verify
  if (( FAIL == 0 )); then
    gate 'checkpoint and clean Git trees' "$MOBILE" python3 scripts/qa-7b-worktree.py commit
