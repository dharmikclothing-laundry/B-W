...
LOG_DIR="$(mktemp -d "${TMPDIR:-/tmp}/bw-9e.XXXXXX")"
REPORT="$REPORT_DIR/9E-ADMIN-SERVICES-PRICING-$(date +%Y%m%d-%H%M%S).md"
...
finish() {
  ...
  note 'Fictional local catalogue and audit retained inactive. Original local checkout policy restored after acceptance.'
  note "Report: $REPORT"
}
trap 'if [[ -n "$API_PID" ]]; then kill "$API_PID" 2>/dev/null || true; wait "$API_PID" 2>/dev/null || true; fi' EXIT
printf '# 9E Admin Services, Pricing and Catalogue\n\nRun: %s\n\n' "$(date '+%Y-%m-%d %H:%M:%S %Z')" >"$REPORT"
note 'Scope: Admin-only category/service management and availability, versioned service prices and checkout policy, separate minimum-order threshold, unsold-package eligibility, audited changes and historical order snapshots.'
...
gate 'clean 9D baselines and reviewed 9E files' "$MOBILE" python3 scripts/qa-9e-worktree.py verify
if (( FAIL > 0 )); then finish; exit 1; fi
note '## Changed files'
python3 "$MOBILE/scripts/qa-9e-worktree.py" verify | rg '^(backend|mobile): ' | sed 's/^/- /' >>"$REPORT"
...
gate 'runner and fixture syntax' "$ROOT" bash -c 'bash -n bw mobile/scripts/admin-catalogue-milestone.sh && node --check backend/scripts/9e-functional-acceptance.cjs && python3 -c '\''import ast,pathlib; ast.parse(pathlib.Path("mobile/scripts/qa-9e-worktree.py").read_text())'\'''
gate 'backend catalogue and checkout pricing targeted tests' "$BACKEND" npm test -- --runInBand --watchman=false admin-catalogue.service.spec.ts order-pricing.spec.ts orders.service.spec.ts services.service.spec.ts
gate 'backend HTTP Admin RBAC integration' "$BACKEND" npm run test:e2e -- --runInBand --watchman=false
gate 'mobile Admin catalogue and checkout pricing tests' "$MOBILE" npm test -- --runInBand --watch=false --watchman=false --silent adminCatalogueApi AdminCatalogueScreen orderPricing OrderReviewScreen
...
if lsof -tiTCP:3001 -sTCP:LISTEN >/dev/null 2>&1; then
  ...
  gate 'local 9E API health' "$ROOT" bash -c 'for i in {1..30}; do curl -fsS --max-time 2 http://127.0.0.1:3001/v1/health >/dev/null 2>&1 && exit 0; sleep 2; done; exit 1'
  if curl -fsS --max-time 2 http://127.0.0.1:3001/v1/health >/dev/null 2>&1; then
    gate 'fictional Admin catalogue, prices, package eligibility, policy and audit acceptance' "$BACKEND" node scripts/9e-functional-acceptance.cjs
  fi
fi
note '## Final checkpoint'
gate 'final changed-file and diff review' "$MOBILE" python3 scripts/qa-9e-worktree.py verify
if (( FAIL == 0 )); then
  if gate 'checkpoint and clean Git trees' "$MOBILE" python3 scripts/qa-9e-worktree.py commit; then CHECKPOINT='CREATED'; fi
fi
finish
(( FAIL == 0 ))
