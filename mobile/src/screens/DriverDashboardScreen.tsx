...
function JobQueue({title, jobs, onJob}: {title: string; jobs: DriverJob[]; onJob: (id: string) => void}) {
  const activeJobs = jobs.filter(job => job.assignmentStatus !== 'completed');
  return <View style={styles.section}>
    <Text style={styles.heading}>{title}</Text>
    {activeJobs.length === 0 ? <View style={styles.empty}><Text style={styles.emptyTitle}>All clear</Text><Text style={styles.muted}>No {title.toLowerCase()} assigned.</Text></View> : activeJobs.map(job =>
      <View key={job.id} style={styles.card}>
        ...
        <Text style={styles.orderId}>Order ID: {job.orderNumber}</Text>
        <Text style={styles.meta}>Facility: {job.facility?.name ?? 'Facility unavailable'}</Text>
