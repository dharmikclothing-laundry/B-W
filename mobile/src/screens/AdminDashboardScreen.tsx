...
const statusLabel = (value: string) => value
  .split('_')
  .map(part => part.toLowerCase() === 'otp' ? 'OTP' : `${part.charAt(0).toUpperCase()}${part.slice(1)}`)
  .join(' ');
  ...
  return <SafeAreaView style={styles.page}><ScrollView contentContainerStyle={styles.content}
    ...
    {dashboard ? <>
      ...
      <View style={styles.statusCard}>{dashboard.ordersByStatus.length ? dashboard.ordersByStatus.map(row =>
        <View key={row.status} style={styles.statusRow}><Text style={styles.statusName}>{statusLabel(row.status)}</Text><Text style={styles.statusCount}>{row.count}</Text></View>)
