...
describe('OrdersService checkout rewards', () => {
  function fixture(minimumOrderAmount = 0) {
    const from = jest.fn((table: string) => {
      ...
      const chain: any = {};
      for (const method of ['select', 'eq', 'lte', 'or', 'order', 'limit', 'is']) chain[method] = jest.fn(() => chain);
    ...
    const service = new OrdersService({admin: {from, rpc}} as any, {
      applyCoupon: jest.fn().mockResolvedValue({offerId: 'offer-1', discount: 10}),
    } as any);
    jest.spyOn(service, 'getById').mockResolvedValue({id: 'order-1'} as any);
    const dto: any = {
      pickupAddressId: 'address-1', deliveryAddressId: 'address-1',
      pickupScheduledAt: new Date(Date.now() + 86400000).toISOString(),
      pickupSlotLabel: 'Morning', paymentMethod: 'cash_on_delivery', termsAccepted: true,
      items: [{serviceId: 'service-1', itemName: 'Shirt', quantity: 1}],
      couponCode: 'SAVE10', loyaltyPointsToRedeem: 1000,
    };
  ...
  it('creates ordinary orders through the same atomic procedure', async () => {
    ...
    await expect(f.service.create('profile-1', f.dto)).resolves.toMatchObject({id: 'order-1'});
    expect(f.rpc).toHaveBeenCalledWith('create_customer_order_with_rewards_atomic',
...
describe('OrdersService order retry', () => {
  ...
  function fixture(hash: string) {
    ...
    const prior = {data: {id: 'order-1', idempotency_request_hash: hash}, error: null};
    const from = jest.fn((table: string) => {
  ...
  it('returns the original customer order for an identical retry', async () => {
    ...
    const hash = createHash('sha256').update(JSON.stringify({...dto, idempotencyKey: undefined})).digest('hex');
    const {service, from} = fixture(hash);
    await expect(service.create('profile-1', dto)).resolves.toEqual({id: 'order-1'});
  ...
  it('rejects reuse of a key for a different checkout', async () => {
    ...
    await expect(service.create('profile-1', dto)).rejects.toBeInstanceOf(ConflictException);
  });
});
  ...
  it('returns a bad request when the database rejects completion', async () => {
    const f = fixture({
      ...
      error: {
        code: '23514',
        message:
          'The delivery claim period is still active',
      },
    });
