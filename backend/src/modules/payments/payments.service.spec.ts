...
describe('PaymentsService', () => {
  ...
  it(
    'requires a matching captured provider payment before confirming the order',
    async () => {
      const paymentOrder = {
        id: 'local-payment-1',
        order_id: 'order-1',
  ...
  it(
    'rejects a provider payment that does not match the payment order',
    async () => {
      const paymentOrder = {
        id: 'local-payment-1',
        order_id: 'order-1',
  ...
  it(
    'persists the required webhook event IDs and marks the event processed',
    async () => {
      process.env.RAZORPAY_WEBHOOK_SECRET =
        'webhook-secret';
      ...
      const rpc = jest.fn(
        async (name: string) => {
          if (
            name ===
            'register_payment_webhook_event_atomic'
          ) {
            return {
              data: {
                claimed: true,
  ...
  it(
    'rejects webhook verification when its secret is missing',
    async () => {
      delete process.env.RAZORPAY_WEBHOOK_SECRET;
  ...
  it('returns a retryable error while another webhook worker holds the lease', async () => {
    ...
    await expect(service.webhook(payload, signature, 'event-1')).rejects.toThrow(
